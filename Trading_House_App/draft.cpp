...
class Member
{
    ...
    public:
            ...
            if(listOfrequests.empty() != 1)
            {
                        ...
                        for(auto each : Each->listOfrequests)
                        {
                            ...
                            if(input_index == std::to_string(i))
                            {
                                ...
                                if(each->creditPoint >= rentingCreditPoint && each->occupierRatingScore_Avg >= minimum_occupierRatingScore_Avg && each->houseOccupy->username == "NULL")
                                {
                                    ...
                                    //Remove all the exist request in other list of members
                                    #if 1
                                    for(auto data : Database)
                                    {
                                        int i = 0;
                                        for(auto request : data->listOfrequests)
                                        {
                                            if(request->userName == temp)
                                            {
                                                data->listOfrequests.erase(data->listOfrequests.begin() + i);
                                            }                                           
                                            i++;
                                        }
                                    }
                ...
                for(auto each: Database)
                {
                    if(each->userName == OccupyMem->userName){
                        ...
                        each->houseOccupy->fullname = fullName;
        ...
        void PutHouseToRent()
        {
            ...
            {
                cout << "Already has member occupied!\n";
            }
        ...
        void showHouseAvailable()
        {
            ...
            for(auto each : Database)
            {
                ...
                for(auto request : each->listOfrequests)
                {
                    ...
                    {
                        alreadyRequest = each->userName;
                    }   
                }
                

                if((each->houseOwn)->available == "Available" && each->userName != userName && each->userName != alreadyRequest)
                {
       
                    cout <<"\n" << i <<  ") House of " << each->fullName << ". Location at " << (each->houseOwn)->location << ".\n"
            ...
            if(choice == "Y") 
            {
                ...
                for(auto each : Database)
                {
                    for(auto request : each->listOfrequests)
                    {
                        if((each->houseOwn)->available == "Available" && each->userName != userName && request->userName != userName)
                        {
                            index++;
                        }
                        if(input_index == std::to_string(i) && input_index != "0")
                        {
            ...
            for(auto each : Database)
            {
                ...
                if(each->userName == userName)
                {
                    ...
                    if(listOfreviews.empty() != 1)
                    {
                        ...
                        cout << "You dont have any reviews!\n";
